...
Draw_t *draw_create_x11(Display *dpy, int screen, Window root, unsigned int w, unsigned int h, Visual *visual, unsigned int depth, Colormap cmap, int protocol) {
    Draw_t *draw = static_cast<Draw_t*>(draw_calloc(1, sizeof(Draw_t)));
...
void draw_create_surface_wl(Draw_t *draw, void *data, int32_t w, int32_t h) {
    ...
    draw->w = w;
    draw->h = h;
    draw->surface = cairo_image_surface_create_for_data(static_cast<unsigned char*>(draw->data), CAIRO_FORMAT_ARGB32, draw->w, draw->h, draw->w * 4);
...
static Fnt *font_create(Draw_t *draw, const char *fontname) {
    ...
    font = static_cast<Fnt*>(draw_calloc(1, sizeof(Fnt)));
    font->dpy = draw->dpy;
...
void draw_arrow(Draw_t *draw, int x, int y, unsigned int w, unsigned int h, int direction, int slash, char *prevcol, char *nextcol, int prevalpha, int nextalpha) {
    ...
    if (draw->protocol) {
        sf = cairo_image_surface_create_for_data(static_cast<unsigned char*>(draw->data), CAIRO_FORMAT_ARGB32, draw->w, draw->h, draw->w * 4);
    } else {
...
void draw_set_img(Draw_t *draw, void *data, int w, int h) {
    ...
    draw->img_data = data;
    draw->img_surface = cairo_image_surface_create_for_data(static_cast<unsigned char*>(draw->img_data), CAIRO_FORMAT_ARGB32, w, h, w * 4);
}
...
void draw_save_screen(Draw_t *draw, const char *file) {
    ...
    if (cairo_surface_write_to_png(draw->surface, file)) {
        fprintf(stderr, "majorna: failed to write file %s\n", file);
    }
}
